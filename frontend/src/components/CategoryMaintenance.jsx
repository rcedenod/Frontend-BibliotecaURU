import { useState, useEffect } from "react";
import "./styles/CategoryMaintenance.css"

function CategoryMaintenance() {
  const [categories, setCategories] = useState([]); // Lista de perfiles
  const [error, setError] = useState("");
  const [showAddPopup, setShowAddPopup] = useState(false);
  const [newCategoryName, setNewCategoryName] = useState("");
  const [editingCategory, setEditingCategory] = useState(null);
  const [editCategoryName, setEditCategoryName] = useState("");
  const [selectedCategories, setSelectedCategories] = useState([]); // IDs seleccionados
  const [showConfirmDelete, setShowConfirmDelete] = useState(false);
  const [showWarningDelete, setShowWarningDelete] = useState(false);
  const [searchTerm, setSearchTerm] = useState(""); // Término de búsqueda

  // Paginación
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 6;

  const filteredCategories = categories.filter(c =>
    c.category.toLowerCase().includes(searchTerm.toLowerCase())
  );

  // Calcular índices para la paginación
  const indexOfLastItem = currentPage * itemsPerPage;
  const indexOfFirstItem = indexOfLastItem - itemsPerPage;
  const currentCategories = filteredCategories.slice(indexOfFirstItem, indexOfLastItem);
  const totalPages = Math.ceil(filteredCategories.length / itemsPerPage);

  // Función para cambiar de página
  const paginate = (pageNumber) => {
    setCurrentPage(pageNumber);
  };

  // Función para ir a la página anterior
  const goToPreviousPage = () => {
    if (currentPage > 1) setCurrentPage(currentPage - 1);
  };

  // Función para ir a la página siguiente
  const goToNextPage = () => {
    if (currentPage < totalPages) setCurrentPage(currentPage + 1);
  };

  // Función para obtener perfiles del backend
  const fetchCategories = async () => {
    try {
      const response = await fetch("http://localhost:3000/ToProcess", {
        method: "POST",
        mode: "cors",
        cache: "default",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          objectName: "CategoryBO",
          methodName: "getCategories",
          params: {}
        })
      });
      const data = await response.json();
      if (data.sts) {
        setCategories(data.data);
      } else {
        setError(data.msg || "Error al obtener las categorias");
      }
    } catch (err) {
      console.error("Error al obtener las categorias:", err);
      setError("Error de conexión al servidor");
    }
  };

  useEffect(() => {
    fetchCategories();
  }, []);

  // Manejar selección individual
  const handleCheckboxChange = (id, checked) => {
    if (checked) {
      setSelectedCategories(prev => [...prev, id]);
    } else {
      setSelectedCategories(prev => prev.filter(item => item !== id));
    }
  };

  // Seleccionar o deseleccionar todos
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const allIds = categories.map(category => category.id_category);
      setSelectedCategories(allIds);
    } else {
      setSelectedCategories([]);
    }
  };

  // Función para eliminar perfiles (muestra popup)
  const handleDeleteSelected = async () => {
    if (selectedCategories.length === 0) {
      setShowWarningDelete(true);
      return;
    }
    setShowConfirmDelete(true);
  };

  const handleConfirmDelete = async () => {
    setShowConfirmDelete(false);
    try {
      const response = await fetch("http://localhost:3000/ToProcess", {
        method: "POST",
        mode: "cors",
        cache: "default",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          objectName: "CategoryBO",
          methodName: "deleteCategories",
          params: { ids: selectedCategories }
        })
      });
      const data = await response.json();
      if (data.sts) {
        fetchCategories();
        setSelectedCategories([]);
      } else {
        alert(data.msg || "Error al eliminar categorias");
      }
    } catch (error) {
      console.error("Error al eliminar categorias:", error);
      alert("Error de conexión al servidor");
    }
  };

  const handleCancelDelete = () => {
    setShowConfirmDelete(false);
  };

  const openEditPopup = (category) => {
    setEditingCategory(category);
    setEditCategoryName(category.category);
  };

  const handleUpdateCategory = async () => {
    if (!editCategoryName.trim()) {
      alert("El nombre del perfil no puede estar vacío");
      return;
    }
    try {
      const response = await fetch("http://localhost:3000/ToProcess", {
        method: "POST",
        mode: "cors",
        cache: "default",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          objectName: "CategoryBO",
          methodName: "updateCategory",
          params: { id_category: editingCategory.id_category, categoryName: editCategoryName }
        })
      });
      const data = await response.json();
      if (data.sts) {
        fetchCategories();
        setEditingCategory(null);
        setEditCategoryName("");
      } else {
        alert(data.msg || "Error al actualizar la categoria");
      }
    } catch (error) {
      console.error("Error al actualizar categoria:", error);
      alert("Error de conexión al servidor");
    }
  };

  const handleAddcategory = async () => {
    if (!newCategoryName.trim()) {
      alert("Ingresa un nombre para la categoria");
      return;
    }
    try {
      const response = await fetch("http://localhost:3000/ToProcess", {
        method: "POST",
        mode: "cors",
        cache: "default",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          objectName: "CategoryBO",
          methodName: "createCategory",
          params: { categoryName: newCategoryName }
        })
      });
      const data = await response.json();
      if (data.sts) {
        fetchCategories();
        setShowAddPopup(false);
        setNewCategoryName("");
      } else {
        alert(data.msg || "Error al crear el perfil");
      }
    } catch (error) {
      console.error("Error al crear perfil:", error);
      alert("Error de conexión al servidor");
    }
  };

  return (
    <div className="categories-maintenance">
      <div className="categories-header">
        <h1>Mantenimiento de categorias</h1>
        <div className="categories-actions">
          <span className="icon-trash" title="Eliminar seleccionados" onClick={handleDeleteSelected}>
            &#128465;
          </span>
          <span className="icon-add" title="Agregar nueva categoria" onClick={() => setShowAddPopup(true)}>
            + Nueva categoria
          </span>
        </div>
      </div>

      {/* Barra de búsqueda */}
      <div className="search-bar-users">
        <input 
          type="text"
          value={searchTerm}
          onChange={(e) => setSearchTerm(e.target.value)}
          placeholder="Buscar categoria..."
        />
      </div>

      {error && <p style={{ color: "red" }}>{error}</p>}

      <div className="categories-table-container">
        <table className="categories-table">
          <thead>
            <tr>
              <th>
                <label className="custom-checkbox">
                <input 
                  type="checkbox" 
                  onChange={handleSelectAll} 
                  checked={categories.length > 0 && selectedCategories.length === categories.length}
                />
                <span></span>
                </label>
              </th>
              <th>Id</th>
              <th>Categoria</th>
              <th>Editar</th>
            </tr>
          </thead>
          <tbody>
            {currentCategories.length > 0 ? (
              currentCategories.map((category) => (
                <tr key={category.id_category}>
                  <td>
                    <label className="custom-checkbox">
                      <input 
                        type="checkbox" 
                        checked={selectedCategories.includes(category.id_category)}
                        onChange={(e) => handleCheckboxChange(category.id_category, e.target.checked)}
                      />
                      <span></span>
                    </label>
                  </td>
                  <td>{category.id_category}</td>
                  <td>{category.category}</td>
                  <td>
                    <span className="edit-icon" title="Editar" onClick={() => openEditPopup(category)}>
                      &#9998;
                    </span>
                  </td>
                </tr>
              ))
            ) : (
              <tr>
                <td colSpan="4">No hay categorias disponibles</td>
              </tr>
            )}
          </tbody>
        </table>
      </div>

      {/* Paginación */}
      <div className="pagination">
        <button onClick={goToPreviousPage} disabled={currentPage === 1}>
          &#8592;
        </button>
        {[...Array(totalPages)].map((_, i) => (
          <button
            key={i + 1}
            className={currentPage === i + 1 ? "active" : ""}
            onClick={() => paginate(i + 1)}
          >
            {i + 1}
          </button>
        ))}
        <button onClick={goToNextPage} disabled={currentPage === totalPages}>
          &#8594;
        </button>
      </div>

      {showAddPopup && (
        <div className="popup-overlay">
          <div className="popup">
            <h2>Agregar nueva categoria</h2>
            <label htmlFor="objectDropdown">Nombre</label>
            <input
              type="text"
              value={newCategoryName}
              onChange={(e) => setNewCategoryName(e.target.value)}
              placeholder="Nombre de la categoria"
            />
            <div className="popup-actions">
              <button onClick={handleAddcategory}>Agregar</button>
              <button onClick={() => setShowAddPopup(false)}>Cancelar</button>
            </div>
          </div>
        </div>
      )}

      {editingCategory && (
        <div className="popup-overlay">
          <div className="popup">
            <h2>Editar categoria</h2>
            <label htmlFor="objectDropdown">Nombre</label>
            <input
              type="text"
              value={editCategoryName}
              onChange={(e) => setEditCategoryName(e.target.value)}
              placeholder="Nuevo nombre de la categoria"
            />
            <div className="popup-actions">
              <button onClick={handleUpdateCategory}>Actualizar</button>
              <button onClick={() => setEditingCategory(null)}>Cancelar</button>
            </div>
          </div>
        </div>
      )}

      {showConfirmDelete && (
        <div className="popup-overlay">
          <div className="popup">
            <h2>Confirmar eliminacion</h2>
            <p>¿Está seguro de eliminar las categorias seleccionadas?</p>
            <div className="popup-actions">
              <button onClick={handleConfirmDelete}>Confirmar</button>
              <button onClick={handleCancelDelete}>Cancelar</button>
            </div>
          </div>
        </div>
      )}

      {showWarningDelete && (
        <div className="popup-overlay">
          <div className="popup">
            <h2>Advertencia</h2>
            <p>No hay categorias seleccionadas para eliminar.</p>
            <div className="popup-actions">
              <button onClick={() => setShowWarningDelete(false)}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
    </div>
  );
}

export default CategoryMaintenance;